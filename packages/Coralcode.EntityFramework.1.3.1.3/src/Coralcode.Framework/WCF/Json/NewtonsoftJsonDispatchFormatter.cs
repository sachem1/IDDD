...
namespace Coralcode.Framework.WCF.Json
{
    public class NewtonsoftJsonDispatchFormatter : IDispatchMessageFormatter
    {
        ...
        public Message SerializeReply(MessageVersion messageVersion, object[] parameters, object result)
        {
            ...
            //}
            object formatResult;
            if (result is BaseMessage)
                formatResult = result;
            else
                formatResult = new ResultMessage(ResultState.Success,data:result);

            Message replyMessage = Message.CreateMessage(messageVersion, operation.Messages[1].Action, new RawBodyWriter(formatResult));
